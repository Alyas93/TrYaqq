...
# مزامنة الفرعين بلا إنترنت — `server/lan.js`

جهاز واحد في الصيدلية يشغّل الخادم، وبقية الأجهزة تتصل به عبر الواي فاي المحلي.

```bash
node server/lan.js          # المنفذ 8787
```
يطبع عناوين الجهاز، ضع أحدها في كل جهاز:
**الإعدادات ← رابط المزامنة** مثل `http://192.168.1.50:8787/`

- البيانات في `server/tiryaq-sync.json` مع نسخة يومية `.bak.json`.
- الكتابة ذرّية (ملف مؤقت ثم إعادة تسمية) فلا يبقى ملف نصف مكتوب لو انقطعت الكهرباء.
- يرفض أي محتوى ليس JSON سليماً.
- على الهاتف: ثبّت Termux ثم `pkg install nodejs` وشغّله بنفس الأمر.

**لا تفتح منفذ هذا الخادم على الإنترنت** — هو لشبكة الصيدلية المغلقة فقط، بلا تشفير ولا مصادقة.

## ما الذي يُزامَن فعلاً
| يُزامَن | لا يُزامَن |
|---|---|
| الأسماء والأسعار والباركود | كمية المخزون |
| الزبائن والديون (بدمج السجل) | التشغيلات وتواريخ الصلاحية |
| الفواتير (مفتاحها الفرع + الرقم) | جلسة الجرد |

الكمية تبقى لكل فرع لأن رصيد رفّ الفرع الأول ليس رصيد الثاني.
